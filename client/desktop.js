// dsh-pet-StatusLight — DSH Desktop client implementation.
// Loaded only by client/client.js after detecting the Desktop client service.
...
window.__ModuleLoader__.load({
  id: 'dsh-pet-statuslight/desktop',
  factory: (require) => {
    var module = { exports: {} }
...
`
    ...
    const StatusLight = () => {
      ...
      const [pageActive, setPageActive] = React.useState(() => {
        try { return typeof document === 'undefined' || (document.visibilityState === 'visible' && document.hasFocus()) } catch (e) { return false }
      })
      ...
      const desktopModeRef = React.useRef(null)
      ...
      React.useEffect(() => {
        const tick = () => {
          ...
          apiCall('state?since=' + since).then((res) => {
            if (!res) return
            setSnap(res)
            // 原生桌面通知全部关闭时，DSH Desktop 的气泡行为回退为普通 dsh web。
            const desktopMode = !!(res.desktopHost && res.desktopNotificationsEnabled !== false)
            const previousDesktopMode = desktopModeRef.current
            desktopModeRef.current = desktopMode
            // 从 Desktop 专用模式切到 web 回退模式时补一次全量快照，使此前因
            // 无跳转链接而整体隐藏的通知能恢复为 600 秒「查看详细」。
            if (previousDesktopMode === true && !desktopMode && !useFull) pendingFullRef.current = true
            ...
            if (res.notifications && res.notifications.length) {
              ...
              for (const n of res.notifications) {
                ...
                merged[key] = { ...n, key, born: desktopMode && existing ? existing.born : nowTick, ttl: desktopMode && !n.canJump ? 10 : 1200 }
              }
              itemsRef.current = Object.values(merged).sort((a, b) => b.seq - a.seq).slice(0, 1)
              setItems(itemsRef.current)
            }
            // 原生通知开启时，仅保留当前可跳转通知的 600 s 展示期；不可跳转状态
            // 使用短 TTL 清理隐藏缓存，聊天框与链接不会出现一方单独残留。
            if (desktopMode && itemsRef.current.length) {
              const adjusted = itemsRef.current.map((n) => {
                const canJump = !!(res.desktopClientActive && res.currentViewedSession && n.agentId !== res.currentViewedSession)
                return { ...n, canJump, ttl: canJump ? 1200 : 10 }
              })
              itemsRef.current = adjusted
              setItems(adjusted)
            } else if (res.desktopHost && itemsRef.current.length) {
              const adjusted = itemsRef.current.map((n) => ({ ...n, canJump: true, ttl: 1200 }))
              itemsRef.current = adjusted
              setItems(adjusted)
            }
        ...
        const onPresence = () => {
          let active = false
          try { active = typeof document === 'undefined' || (document.visibilityState === 'visible' && document.hasFocus()) } catch (e) {}
      ...
      const jump = (n) => {
        if (snap && snap.desktopHost) {
          // Desktop 先同步移除本地气泡，随后立即调用会话导航；Host 已读请求放到最后，
          // 避免 fetch 启动位于 sessions.open 前面，同时保持点击后气泡即时消失。
          closeItem(n.key)
          performJump(n)
          apiCall('dismiss?seq=' + n.seq)
          return
      ...
      const desktopMode = !!(snap && snap.desktopHost && snap.desktopNotificationsEnabled !== false)
      const visibleItems = desktopMode
        ? items.filter((n) => pageActive && snap.desktopClientActive && snap.currentViewedSession && n.agentId !== snap.currentViewedSession)
        : items
      ...
      return React.createElement('div', { className: 'sl-widget' },
        React.createElement('div', { className: 'sl-chat-stack' },
          visibleItems.map((n) => React.createElement('div', { key: n.key, className: 'sl-chatbox' },
            ...
            React.createElement('div', { className: 'sl-chatbox-body' },
              ...
              React.createElement('button', { className: 'sl-chatbox-link', onClick: () => jump(n) }, desktopMode ? '跳转会话' : '查看详细')
            ),
    ...
    //  - Desktop 真正重新获得焦点且仍在同一会话 → read=1：用户已手动返回目标会话，
    //    将该会话通知标记为已读并主动关闭聊天框；
    //  - Desktop 原生通知开启且在后台：保留当前会话并上报 active=0，以隐藏「跳转会话」；
    //  - Desktop 原生通知关闭：Host 按普通 web 处理可见性、已读状态和「查看详细」；
    //  - 普通 web 页面后台/离开会话 → view?session=：清除当前会话（聊天框照常弹出）。
    const reportView = (sid, read) => {
      try {
        const visible = typeof document === 'undefined' || document.visibilityState === 'visible'
        const active = visible && (typeof document === 'undefined' || typeof document.hasFocus !== 'function' || document.hasFocus())
        const markRead = !!read && active
        let q = 'session='
        if (sid) {
          q = 'session=' + encodeURIComponent(sid) + '&visible=' + (visible ? '1' : '0') + '&active=' + (active ? '1' : '0') + (markRead ? '&read=1' : '')
        }
    ...
    const CurrentSessionReporter = (props) => {
      ...
      React.useEffect(() => {
        ...
        reportView(sid, true)
        // focus/blur/visibilitychange 都请求已读，但 reportView 只在 active=true 时真正附加 read=1；
        // 因此失焦、最小化和隐藏只更新状态，手动返回前台才关闭当前会话聊天框。
        const onVis = () => reportView(sidRef.current, true)
        if (typeof document !== 'undefined' && typeof document.addEventListener === 'function') {
