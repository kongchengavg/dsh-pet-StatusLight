...
模型思考、出错、完成任务时，角色表情实时切换；完成、出错、向你提问时，角色头顶弹出专属聊天框。普通 dsh web 使用基于 1.0.37 的「查看详细」跳转，并在用户从后台返回弹窗目标会话时主动关闭聊天框；DSH Desktop 启用原生桌面通知时，只有位于前台且通知来自其他会话才同时显示聊天框与「跳转会话」，并保留 600 秒，其余状态二者同时隐藏；关闭原生桌面通知后则完整回退为聊天框与「查看详细」同时显示或消失。就算浏览器最小化了也不怕——系统级置顶小窗继续陪着你。
...
- 💬 **角色专属聊天框**：任务完成、出错、向你提问时弹出气泡提示；普通 dsh web 可点「查看详细」跳转。DSH Desktop 中链接与聊天框始终同时出现和消失：启用原生通知时仅在前台不同会话显示「跳转会话」并保留 600 秒，关闭原生通知时显示「查看详细」并采用与 dsh web 相同的时长。
...
- **聊天框**：普通 dsh web 使用基于 1.0.37 的「查看详细」跳转与显示时长；从后台手动返回弹窗目标会话时，聊天框会主动关闭。DSH Desktop 也会在用户手动返回弹窗目标会话时主动关闭聊天框；聊天框与链接始终同时出现和消失。启用原生桌面通知时，仅在 Desktop 前台且通知来自其他会话时显示「跳转会话」并保留 600 秒，失焦、最小化、隐藏或当前已是目标会话时二者同时隐藏。关闭原生桌面通知时，即使 Desktop 失焦、最小化或隐藏，也同时显示聊天框与「查看详细」，时长、已读和跳转规则与 dsh web 相同。点击链接、手动进入目标会话或点击「×」都会关闭聊天框。
