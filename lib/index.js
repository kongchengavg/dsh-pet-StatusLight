...
export function apply(ctx) {
    ...
    function pushNotification(kind, agent) {
      ...
      if (currentViewedSession && n.agentId === currentViewedSession) dismissed.add(n.seq)
    }
          ...
          try {
            ...
            if (parts[0] === 'api') {
              ...
              if (cmd === 'view') {
                ...
                //    之后切走也不补发；
                //  - 无 read（页面回前台/可见性变化，仍在同一会话）：仅更新 currentViewedSession，
    ...
    const PS1 = '\uFEFF' + 'param(\n' + '  [string]$Api = "http://127.0.0.1:3080/statuslight/api",\n' + '  [string]$Config = ""\n' + ')\n' +
      ...
      '$timer.Add_Tick({ $script:tick++; $s = Read-Json; $want = if ($null -ne $s) { $s.window } elseif ($null -ne $script:state) { $script:state.window } else { $true }; if ($null -eq $want) { $want = $true }; Sync-Window ([bool]$want); if ($null -eq $s) { return }; $script:state = $s; if ($s.image) { Set-CharImage ([string]$s.image) }; if ($null -ne $s.chatOffset) { Apply-Offset ([int]$s.chatOffset) }; if ($script:lastNotif -ne $null -and $script:lastNotif.kind -ne "question" -and $s.runningAgents -and @($s.runningAgents) -contains [string]$script:lastNotif.agentId) { Hide-Chat }; if ($script:lastNotif -ne $null -and $null -ne $s.notifications) { $still = $false; foreach ($nn in @($s.notifications)) { if ([int]$nn.seq -eq [int]$script:lastNotif.seq) { $still = $true; break } }; if (-not $still) { Hide-Chat; $script:lastNotif = $null } }; if ($s.notifications -and $s.notifications.Count -gt 0) { $n = $s.notifications[$s.notifications.Count - 1]; if ([int]$n.seq -gt $script:lastSeq) { $script:lastSeq = [int]$n.seq; $script:lastNotif = $n; if ($n.chatbox) { Set-ChatImage ([string]$n.chatbox) }; $len = [int]$n.text.Length; if ($len -ge 20) { $chatText.FontSize = 10 } else { $chatText.FontSize = 12 }; $chatText.Text = [string]$n.text; $chatImg.Visibility = [System.Windows.Visibility]::Visible; $chatText.Visibility = [System.Windows.Visibility]::Visible; $viewLink.Visibility = [System.Windows.Visibility]::Visible; $closeBtn.Visibility = [System.Windows.Visibility]::Visible; $script:chatUntil = $script:tick + 1200 } elseif ($n.chatbox -ne $null -and [string]$n.chatbox -ne $script:lastChat) { $script:lastNotif = $n; if ($n.chatbox) { Set-ChatImage ([string]$n.chatbox) } } }; if ($script:chatUntil -gt 0 -and $script:tick -ge $script:chatUntil) { Hide-Chat } })\n' +
