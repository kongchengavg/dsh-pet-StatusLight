...
export function apply(ctx) {
    ...
    async function loadConfig() {
      if (!fs || !baseDir) return
      // 仅当包位于只读的 node_modules（npm 全局分发）时跳过配置读写；
      // link 安装（项目目录、符号链接）与工作区布局仍正常持久化配置。
      if (PACKAGE_BASE_RAW && baseDir === PACKAGE_BASE_RAW && baseDir.indexOf('/node_modules/') >= 0) return
      try {
    ...
    const PS1 = '\uFEFF' + 'param(\n' + '  [string]$Api = "http://127.0.0.1:3080/statuslight/api",\n' + '  [string]$Config = ""\n' + ')\n' +
      ...
      '$timer.Add_Tick({ $script:tick++; $s = Read-Json; $want = if ($null -ne $s) { $s.window } elseif ($null -ne $script:state) { $script:state.window } else { $true }; if ($null -eq $want) { $want = $true }; Sync-Window ([bool]$want); if ($null -eq $s) { return }; $script:state = $s; if ($s.image) { Set-CharImage ([string]$s.image) }; if ($null -ne $s.chatOffset) { Apply-Offset ([int]$s.chatOffset) }; if ($script:lastNotif -ne $null -and $script:lastNotif.kind -ne "question" -and $s.runningAgents -and @($s.runningAgents) -contains [string]$script:lastNotif.agentId) { Hide-Chat }; if ($s.notifications -and $s.notifications.Count -gt 0) { $n = $s.notifications[$s.notifications.Count - 1]; if ([int]$n.seq -gt $script:lastSeq) { $script:lastSeq = [int]$n.seq; $script:lastNotif = $n; if ($n.chatbox) { Set-ChatImage ([string]$n.chatbox) }; $len = [int]$n.text.Length; if ($len -ge 20) { $chatText.FontSize = 10 } else { $chatText.FontSize = 12 }; $chatText.Text = [string]$n.text; $chatImg.Visibility = [System.Windows.Visibility]::Visible; $chatText.Visibility = [System.Windows.Visibility]::Visible; $viewLink.Visibility = [System.Windows.Visibility]::Visible; $closeBtn.Visibility = [System.Windows.Visibility]::Visible; $script:chatUntil = $script:tick + 120 } elseif ($n.chatbox -ne $null -and [string]$n.chatbox -ne $script:lastChat) { $script:lastNotif = $n; if ($n.chatbox) { Set-ChatImage ([string]$n.chatbox) } } }; if ($script:chatUntil -gt 0 -and $script:tick -ge $script:chatUntil) { Hide-Chat } })\n' +
    ...
    async function spawnWindow() {
      ...
      try {
        if (!scriptPath) {
        // 仅 npm 全局分发（node_modules 只读）时小窗脚本写到系统临时目录；
        // link 安装（项目目录）直接写到项目根，配置/脚本同目录更直观。
        if (PACKAGE_BASE_RAW && baseDir === PACKAGE_BASE_RAW && baseDir.indexOf('/node_modules/') >= 0) {
          let tmp = '/tmp'
