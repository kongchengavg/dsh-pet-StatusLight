...
//   B. <baseDir>/<角色>（兼容旧布局）
export const name = 'dsh-statuslight'
...
export function apply(ctx) {
    ...
    // ---------- RPC ----------
    ctx.effect(() => harness.handle('status-light/state', async (args) => buildSnapshot(args && args.since)))

    ctx.effect(() => harness.handle('status-light/select-character', async (args) => {
      const folder = args && args.folder ? String(args.folder) : null
      if (!folder) return { ok: false }
      if (!characters.some((c) => c.folder === folder)) await ensureBase()
      const ok = await setCharacter(folder)
      return { ok, character: ok ? { folder, name: charName(folder) } : null }
    }))

    ctx.effect(() => harness.handle('status-light/save-position', async (args) => {
      await ensureBase()
      if (args && typeof args.x === 'number' && typeof args.y === 'number') {
        cfg.position = { x: Math.round(args.x), y: Math.round(args.y) }
        await saveConfig()
        return { ok: true }
      }
      return { ok: false }
    ...
    ctx.effect(() => harness.handle('status-light/window', async (args) => {
      await ensureBase()
      const enabled = !!(args && args.enabled)
      await setWindow(enabled)
      return { ok: true, window: windowEnabled }
    }))

    ctx.effect(() => harness.handle('status-light/dismiss', async (args) => {
      dismissNotification(args && typeof args.seq === 'number' ? args.seq : NaN)
      return { ok: true }
    }))
        ...
        handler: async (req, res) => {
          try {
            ...
            if (parts[0] === 'api') {
              ...
              if (cmd === 'state') { const snap = await buildSnapshot(0); return sendJson(res, snap) }
    ...
    async function spawnWindow() {
      ...
      try {
        if (!scriptPath) scriptPath = baseDir + '/' + WINDOW_SCRIPT
